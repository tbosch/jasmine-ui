...
$.extend( $.support, {
	...
	dynamicBaseTag: false && baseTagTest(), // BUGIX JQM-OC-5: Bse tag results in unneeded page reloads when used with hashpaths!
	...
	$.extend( $.mobile, {
		...
		silentScroll: function( ypos ) {
            ...
            if (! (typeof ypos == 'number')) {
			    // Note: The check for typeof is needed as sometimes
                // a jQuery event is passed as argument!
			    // JQM-OC-5 Hide the address bar also on android. For
                // this we the ypos needs to be set to 1.
...
(function($, undefined ) {
	...
	var $window = $(window),
		...
		path = {
			...
			set: function( path ){
                // BUGIX JQM-OC-5 Use #! instead of # so that the browser does no navigation based on ids!
	...
	$.mobile.changePage = function( to, transition, reverse, changeHash, fromHashChange ){
		...
		function transitionPages() {
			...
			if(transition && (transition !== 'none')){
				...
				// BUGFIX JQM-OC-4 do not use this css class as it leads to
                // uncontrolled ui artefacts when using css animations/transitions
				// addContainerClass('ui-mobile-viewport-transitioning');
                /**
                 * BUGFIX CSS ANIMATIONS:
                 * Use CSS3 Transitions instead of CSS3 Animations
                 * due to flickers back to the start on Android devices.
                 */
                if (transition=='slide') {
                    if (from) {
                        from.addClass('transition '+transition+' out '+ ( reverse ? "reverse" : "" )+' start');
                    }
                    to.addClass($.mobile.activePageClass+' transition '+transition+' in '+ ( reverse ? "reverse" : "" )+' start');
                    window.setTimeout(function() {
                        if (from) {
                            from.addClass('end');
                        }
                        to.addClass('end');
                    // Note: The wait time of 20 ms is needed for iOs to do the transition correctly!
                    },20);
                    to.transitionComplete(function() {
                        if( from ){
                            from.removeClass( $.mobile.activePageClass );
                        }
                        to.add(from).removeClass('transition in out start end reverse '+transition);
...
$.fixedToolbars = (function(){
	...
	$('.ui-page').live('pagebeforeshow', function(event, ui){
		...
		var prevFooter = prevPage && prevPage.find( ":jqmData(role='footer')" );
