jasmineui.require(['factory!client/remoteSpecClient'], function (remoteSpecClientFactory) {
    describe('client/remoteSpecClient', function () {
        var mod;
        var d, dui, i, r, be, ae, bl, wf, w;
        var serverMod;
        var noop = function () {
        };
        var callback;
        beforeEach(function () {
            ...
            mod = remoteSpecClientFactory({
                'remote!server/remoteSpecServer':jasmine.createSpy().andReturn(serverMod)
            });
            dui = mod.describeUi;
            d = mod.describe;
            i = mod.it;
        ...
        describe('runs', function () {
            it("should add client defined nodes to the server", function () {
                ...
                mod.executeSpecNode(['describe1', 'it1']);
                expect(serverMod.addClientDefinedSpecNode).toHaveBeenCalledWith('runs', '0', undefined);
            });
            it("should not add client defined nodes to the server when not below the called node from the server", function () {
                ...
                mod.executeSpecNode(['describe1', 'it1', '0']);
                expect(serverMod.addClientDefinedSpecNode).not.toHaveBeenCalled();
            });
        });
        ...
        describe('waitsFor', function () {
            it("should add client defined nodes to the server", function () {
                ...
                mod.executeSpecNode(['describe1', 'it1']);
                expect(serverMod.addClientDefinedSpecNode).toHaveBeenCalledWith('waitsFor', '0', [1234]);
            });
            it("should not add client defined nodes to the server when not below the called node from the server", function () {
                dui('describe1', 'someUrl', function () {
                    i('it1', function () {
        ...
        describe('waits', function () {
            it("should add client defined nodes to the server", function () {
                ...
                mod.executeSpecNode(['describe1', 'it1']);
                expect(serverMod.addClientDefinedSpecNode).toHaveBeenCalledWith('waits', '0', [1234]);
            });
            it("should not add client defined nodes to the server when not below the called node from the server", function () {
                dui('describe1', 'someUrl', function () {
                    i('it1', function () {
